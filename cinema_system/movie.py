...
"""
@file main.py
@brief Головний скрипт для запуску консольного інтерфейсу системи управління кінотеатром.
@details Цей файл містить головну логіку (main loop) для взаємодії
         користувача з системою `CinemaManager`. Він дозволяє переглядати
         фільми, сеанси, бронювати квитки та керувати бронюваннями.

@example
Для запуску інтерактивної консолі виконайте:
@code
python main.py
@endcode
"""
...
def print_movies(movies: list[Movie]):
    """
    @brief Друкує список фільмів у гарному форматі.
    @details Ітерує по списку об'єктів Movie і виводить їхні атрибути
             (назву, рік, режисера, рейтинг, жанри) у консоль.
             Якщо список порожній, виводить повідомлення "Фільмів не знайдено."
    @param movies Список об'єктів `Movie`, які потрібно вивести.
    @return None
    """
    if not movies:
...
def main():
    """
    @brief Головна функція для запуску консольного інтерфейсу.
    @details Ініціалізує `CinemaManager` і запускає нескінченний цикл (REPL),
             що очікує на команди користувача (1-7 або 0 для виходу).
             Обробляє ввід користувача для керування фільмами, сеансами та бронюваннями.
             Внутрішньо обробляє `ValueError` при вводі чисел (напр., для вибору 4 та 5).
    @return None
    """
